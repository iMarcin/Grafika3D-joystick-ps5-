...
namespace grafika3d
{
    //klasa pomocnicza do transformacji
    public abstract class Transformowalne
    {
        private Vec3 position = new Vec3();
        private Vec3 rotation = new Vec3();
        private Vec3 scale = new Vec3(1, 1, 1);
        private Vec3 origin = new Vec3();
        private Transformacja parentTransform = Transformacja.Identity;
        ...
        public Transformacja ParentTransform { get => parentTransform; set { parentTransform = value; transformUpdateNeeded = inverseTransformUpdateNeeded = true; } }



        private Transformacja transform = Transformacja.Identity;
        private bool transformUpdateNeeded = false;
        private Transformacja inverseTransform = Transformacja.Identity;
        private bool inverseTransformUpdateNeeded = false;
        public Transformacja Transform
        {
            get
            {
                if (transformUpdateNeeded)
                {
                    transform = ParentTransform * Transformacja.Identity.Translate(-Origin).Rotate(Rotation).Scale(Scale).Translate(Position);
        ...
        public Transformacja InverseTransform
        {
            get
            {
                if (inverseTransformUpdateNeeded)
                {
                    ...
                    inverseTransform = ParentTransform * new Transformacja(ans11, ans12, ans13, ans14, ans21, ans22, ans23, ans24, ans31, ans32, ans33, ans34, ans41, ans42, ans43, ans44);
